...
async function generateErgaCode(customer_id, sign_date) {
...
export const updateErga = async(req, res) => {
  ...
  // Generate code only if the record doesn't have one yet
  let erga_code = erga.erga_code;
  if (!erga_code) {
    erga_code = await generateErgaCode(
